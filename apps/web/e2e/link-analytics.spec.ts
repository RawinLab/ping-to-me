...
test.describe('Link Analytics - Navigation & Layout', () => {
  ...
  test('LA-001: Can navigate to analytics from links page', async ({ page }) => {
  ...
  test('LA-002: Can navigate to analytics from dropdown menu', async ({ page }) => {
    // Wait for links to load
    await page.waitForSelector('.group.bg-white.rounded-2xl');
  ...
  test('LA-003: Analytics page shows loading state initially', async ({ page }) => {
    ...
    const loadingOrContent = page.locator('text=Loading analytics, text=Total Engagements').first();
...
test.describe('Link Analytics - Link Header Card', () => {
  ...
  test('LA-011: Link header shows short URL', async ({ page }) => {
    ...
    const shortUrlLink = page.locator('a[href*="pingto.me"], a:has-text("pingto.me")').first();
...
test.describe('Link Analytics - Stats Cards', () => {
  ...
  test('LA-024: Weekly change shows trend icon for positive change', async ({ page }) => {
    ...
    const hasTrend = await trendingUp.isVisible() || await trendingDown.isVisible() || await zeroPercent.isVisible();
...
test.describe('Link Analytics - Date Range Selector', () => {
  ...
  test('LA-034: Date range display updates when switching', async ({ page }) => {
    // Get initial date display
    const dateDisplay = page.locator('div:has(svg.lucide-calendar) span').first();
  ...
  test('LA-035: Switching date range triggers data reload', async ({ page }) => {
...
test.describe('Link Analytics - Engagements Chart', () => {
  ...
  test('LA-041: Chart has export button', async ({ page }) => {
    ...
    const chartCard = page.locator('.overflow-hidden').filter({ hasText: 'Engagements' }).first();
    const exportButton = chartCard.locator('button').filter({ has: page.locator('svg.lucide-download') });
...
test.describe('Link Analytics - Locations Chart', () => {
  ...
  test('LA-053: Can switch between Countries and Cities tabs', async ({ page }) => {
    // Click Cities tab
  ...
  test('LA-054: Show more button appears if many locations', async ({ page }) => {
...
test.describe('Link Analytics - Recent Activity Table', () => {
  ...
  test('LA-083: Export button is visible', async ({ page }) => {
    const recentActivityCard = page.locator('text=Recent Activity').locator('..').locator('..');
    const exportButton = recentActivityCard.locator('button').filter({ has: page.locator('svg.lucide-download') });
    await expect(exportButton).toBeVisible();
  });
...
test.describe('Link Analytics - QR Code & Bio Page Cards', () => {
  ...
  test('LA-093: Bio Page card has Create button', async ({ page }) => {
    await expect(page.locator('button:has-text("Create Bio Page")')).toBeVisible();
  });
});
...
test.describe('Link Analytics - Responsive & Loading States', () => {
  ...
  test('LA-100: Page handles invalid link ID gracefully', async ({ page }) => {
    ...
    // One of these should be visible
    await expect(noData.or(loading).or(content)).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Link Analytics - Navigation from Different Sources', () => {
  ...
  test('LA-110: Can navigate from dashboard overview analytics', async ({ page }) => {
...
test.describe('Link Analytics - Data Accuracy', () => {
  ...
  test('LA-121: Weekly change is a valid percentage', async ({ page }) => {
    ...
    const weeklyChange = page.locator('text=Weekly change').locator('..').locator('..').locator('.text-4xl');
