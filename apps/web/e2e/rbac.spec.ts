...
async function isPathAccessible(page: Page, path: string): Promise<boolean> {
  ...
  const hasAccessDenied = await page
    .locator("text=Access Denied, text=Unauthorized, text=403")
...
async function hasVisibleText(
  ...
  return await page
    .locator(`text=${typeof text === "string" ? text : text.source}`)
    .isVisible({ timeout: 2000 })
    .catch(() => false);
}
...
test.describe("RBAC - Role-Based Access Control", () => {
  ...
  test.describe("OWNER Role", () => {
    ...
    test("RBAC-001: can access organization settings", async ({ page }) => {
      ...
      const isAccessible = await isPathAccessible(
        page,
    ...
    test("RBAC-002: can manage all members", async ({ page }) => {
      ...
      const hasAccessDenied = await hasVisibleText(
        page,
      ...
      const hasTeamContent = await page
        .locator('[data-testid="team-members"], text=/member|team/i')
        .first()
  ...
  test.describe("ADMIN Role", () => {
    ...
    test("RBAC-010: can access limited organization settings", async ({
      page,
    }) => {
      // Navigate to organization settings
    ...
    test("RBAC-012: cannot access billing management", async ({ page }) => {
      ...
      const isBillingBlocked =
        hasAccessDenied || !currentUrl.includes("/billing");
  ...
  test.describe("EDITOR Role", () => {
    ...
    test("RBAC-020: cannot access organization settings", async ({ page }) => {
      ...
      const currentUrl = page.url();
      const isOrgSettingsBlocked =
        hasAccessDenied || !currentUrl.includes("/settings/organization");
    ...
    test("RBAC-021: cannot manage team", async ({ page }) => {
      ...
      const currentUrl = page.url();
      const isTeamManagementBlocked =
        hasAccessDenied || !currentUrl.includes("/settings/team");
    ...
    test("RBAC-025: can view organization analytics", async ({ page }) => {
      ...
      const isAnalyticsPage =
        currentUrl.includes("/analytics") || currentUrl.includes("/dashboard");
  ...
  test.describe("VIEWER Role", () => {
    ...
    test("RBAC-031: cannot create links", async ({ page }) => {
      ...
      const isCreateBlocked =
        hasAccessDenied || !currentUrl.includes("/create");
  ...
  test.describe("Permission Enforcement", () => {
    test("RBAC-040: API returns 403 for unauthorized action", async ({
      page,
    }) => {
    ...
    test("RBAC-042: Role change takes effect immediately", async ({ page }) => {
      ...
      const canCreateLinks = !(await hasVisibleText(
        page,
        /Access Denied|Unauthorized/,
      ));
