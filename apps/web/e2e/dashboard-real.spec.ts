...
import { TEST_IDS, TEST_SLUGS, TEST_CREDENTIALS, EXPECTED_ANALYTICS } from './fixtures/test-data';
...
test.describe('Dashboard with Real Data', () => {
  ...
  test('DASH-R001: View dashboard metrics with real data', async ({ page }) => {
    ...
    await expect(page.locator('text=Recent Clicks').or(page.locator('text=Active Links'))).toBeVisible();
  });

  test('DASH-R002: View recent links in dashboard', async ({ page }) => {
    // Check for recent links section
    await expect(page.locator('text=Your Links').or(page.locator('text=Recent Links'))).toBeVisible();
    ...
    if (await linkTable.isVisible()) {
      ...
      await expect(
        page.locator(`text=${TEST_SLUGS.links.recent1}`)
  ...
  test('DASH-R003: View top performing links', async ({ page }) => {
    ...
    const topLinksSection = page.locator('text=Top Performing Links').or(page.locator('text=Top Links'));

    if (await topLinksSection.isVisible()) {
      // Popular link should appear in top performing
      await expect(page.locator(`text=${TEST_SLUGS.links.popular}`)).toBeVisible({ timeout: 5000 });
    }
  });
...
test.describe('Analytics with Real Data', () => {
  ...
  test('ANA-R001: View analytics for popular link', async ({ page }) => {
    ...
    await expect(page.locator('h1, h2').filter({ hasText: /Analytics|Popular Link/ })).toBeVisible({ timeout: 10000 });
  ...
  test('ANA-R002: View device breakdown', async ({ page }) => {
    ...
    const devicesSection = page.locator('.recharts-wrapper, [data-testid="devices-chart"]').first();
  ...
  test('ANA-R003: View country breakdown', async ({ page }) => {
    ...
    await expect(page.locator('text=Top Countries').or(page.locator('text=Countries'))).toBeVisible({ timeout: 10000 });
    ...
    for (const country of countryLabels) {
      if (await page.locator(`text=${country}`).isVisible().catch(() => false)) {
        foundCountry = true;
        break;
      }
    }
  ...
  test('ANA-R004: View referrer breakdown', async ({ page }) => {
    ...
    const referrersSection = page.locator('text=Top Referrers').or(page.locator('text=Referrers'));

    if (await referrersSection.isVisible()) {
      // Should show at least one referrer
      await expect(
        page.locator('text=google.com')
  ...
  test('ANA-R005: View clicks over time', async ({ page }) => {
    ...
    await expect(page.locator('text=Clicks Over Time').or(page.locator('text=Click Trends'))).toBeVisible({ timeout: 10000 });
  ...
  test('ANA-R006: Analytics date range selector', async ({ page }) => {
    ...
    const dateSelector = page.locator('button:has-text("7d"), button:has-text("30d"), button:has-text("90d")');
...
test.describe('Links Management with Real Data', () => {
  ...
  test('LINK-R002: Search for specific link', async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      await expect(page.locator(`text=${TEST_SLUGS.links.popular}`)).toBeVisible();
    }
  });
  ...
  test('LINK-R003: Filter links by tag', async ({ page }) => {
    ...
    // Look for tag filter
    const tagFilter = page.locator('button:has-text("marketing"), [data-testid="tag-filter"]');

    if (await tagFilter.first().isVisible()) {
      await tagFilter.first().click();

      // Should show only marketing tagged links
      await expect(page.locator(`text=${TEST_SLUGS.links.marketing}`)).toBeVisible();
    }
  });
  ...
  test('LINK-R004: View expired link status', async ({ page }) => {
    ...
    const expiredRow = page.locator('tr, [role="row"]').filter({ hasText: TEST_SLUGS.links.expired });

    if (await expiredRow.isVisible()) {
      // Should show expired status indicator
      await expect(expiredRow.locator('text=Expired, .text-red-500, [data-status="expired"]')).toBeVisible();
    }
  });
  ...
  test('LINK-R005: Click through to link analytics', async ({ page }) => {
    ...
    const popularRow = page.locator('tr, [role="row"]').filter({ hasText: TEST_SLUGS.links.popular });

    if (await popularRow.isVisible()) {
      // Click on analytics icon/button
      const analyticsButton = popularRow.locator('a[href*="analytics"], button:has-text("Analytics"), [aria-label*="analytics"]');
...
test.describe('Organization Features with Real Data', () => {
  ...
  test('ORG-R001: View organization in switcher', async ({ page }) => {
    ...
    if (await orgSwitcher.isVisible()) {
      ...
      await expect(page.locator(`text=${TEST_SLUGS.organizations.main}`).or(page.locator('text=E2E Test Organization'))).toBeVisible();
    }
  });
  ...
  test('ORG-R002: View custom domain', async ({ page }) => {
    ...
    await expect(page.locator(`text=${TEST_SLUGS.domains.verified}`)).toBeVisible({ timeout: 10000 });

    // Should show verified status
    const verifiedRow = page.locator('tr, [role="row"]').filter({ hasText: TEST_SLUGS.domains.verified });
    if (await verifiedRow.isVisible()) {
      await expect(verifiedRow.locator('text=Verified, .text-green-500, [data-verified="true"]')).toBeVisible();
    }
  });
});
...
test.describe('Notifications with Real Data', () => {
  ...
  test('NOTIF-R001: View notifications', async ({ page }) => {
    ...
    const notificationBell = page.locator('[aria-label*="notification"], button:has(svg.lucide-bell)');
    ...
    if (await notificationBell.isVisible()) {
      ...
      await expect(
        page.locator('text=Welcome to PingTO.Me')
  ...
  test('NOTIF-R002: Has unread notifications indicator', async ({ page }) => {
    ...
    const unreadBadge = page.locator('[data-testid="unread-count"], .notification-badge, span.bg-red-500');
