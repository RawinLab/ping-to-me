...
const mockAuditLogs = [
  ...
  },
  {
    ...
    changes: {
      before: { targetUrl: "https://old.com" },
      after: { targetUrl: "https://new.com" },
    },
    ipAddress: "192.168.1.1",
    userAgent: "Mozilla/5.0",
...
test.describe("Audit Log Integration (TASK-2.6.25)", () => {
  ...
  test("AUD-001: Link creation creates audit log", async ({ page }) => {
    ...
    const urlInput = page
      ...
      .or(page.locator('input[id="originalUrl"]'))
      ...
      await route.fulfill({
        ...
        body: JSON.stringify([
          ...
          },
          {
            ...
            userId: "user-2",
            role: "EDITOR",
            user: { email: "editor@example.com", name: "Editor" },
          },
        ]),
      });
    });
...
test.describe("Audit Log Viewer (TASK-2.6.26)", () => {
  ...
  test("AUD-011: Filter by action", async ({ page }) => {
    ...
    const linkCreatedOption = page.locator(
      'div[role="option"]:has-text("Link Created")',
    );
  ...
  test("AUD-012: Filter by resource", async ({ page }) => {
    ...
    const linkOption = page
      .locator('div[role="option"]:has-text("Link")')
      .first();
  ...
  test("AUD-013: Filter by date range", async ({ page }) => {
    ...
    const dateSelect = page
      .locator('[data-testid="date-range-select"]')
    ...
    if (await dateSelect.isVisible({ timeout: 5000 })) {
      ...
      const last30Days = page.locator(
        '[role="option"]:has-text("Last 30 days")',
      );
  ...
  test("AUD-014: Filter by status", async ({ page }) => {
    ...
    const successOption = page.locator(
      'div[role="option"]:has-text("Success")',
    );
    if (await successOption.isVisible()) {
      ...
      await expect(
        page
          .locator('button:has-text("Success")')
      ...
      if (route.request().url().includes("/export")) {
        ...
        if (format === "json") {
          await route.fulfill({
            ...
            headers: {
              "Content-Disposition":
                'attachment; filename="audit-logs-export.json"',
            },
            body: JSON.stringify(mockAuditLogs),
          });
        } else {
          const csvContent =
            "Timestamp,User ID,Action,Resource,Status\n" +
            mockAuditLogs
              .map(
                (log) =>
...
test.describe("Audit Log RBAC (TASK-2.6.28)", () => {
  ...
  test("AUD-034: Unauthorized user cannot access audit logs", async ({
    page,
  }) => {
