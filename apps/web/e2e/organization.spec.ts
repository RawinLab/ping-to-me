...
test.describe('Link Organization', () => {
  const mockTags = [
    { id: 'tag-1', name: 'Marketing', color: '#0000ff' }
  ];
  const mockCampaigns = [
    { id: 'camp-1', name: 'Summer Sale', description: 'Summer 2024', _count: { links: 0 } }
  ];
  const mockLinks = [
    { id: 'link-1', originalUrl: 'https://example.com/1', slug: 'link-1', shortUrl: 'http://localhost:3000/link-1', createdAt: new Date().toISOString(), status: 'ACTIVE', tags: [mockTags[0]] }
  ];
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/links?*', async route => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          data: filteredLinks,
          meta: { total: filteredLinks.length, page: 1, limit: 10, totalPages: 1 }
        })
      });
    });
  });
  ...
  test('ORG-001: Create Tag', async ({ page }) => {
    ...
    await page.route('**/tags', async route => {
      if (route.request().method() === 'POST') {
        ...
        await route.fulfill({
          status: 201,
          contentType: 'application/json',
          body: JSON.stringify({ id: 'new-tag', name: 'New Tag', color: '#00ff00' })
        });
      } else {
    ...
    await expect(page.locator('input[placeholder="e.g. Marketing"]')).toBeEmpty();
  });
  ...
  test('ORG-003: Create Campaign', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder="e.g. Summer Sale"]')).toBeEmpty();
  });
