...
test.describe('Short Link Creation', () => {
    ...
    await page.route('**/auth/refresh', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          user: { id: 'mock-user-id', email: 'test@example.com', role: 'OWNER' }
        })
      });
    });
  ...
  test('LINK-003: Create Short Link - Invalid URL', async ({ page }) => {
    ...
    const isInvalid = await page.$eval('input[type="url"]', (el: HTMLInputElement) => !el.checkValidity());
    expect(isInvalid).toBe(true);
  });

  test('LINK-004: Create Short Link - Duplicate Custom Slug', async ({ page }) => {
    await page.route('**/links', async route => {
      await route.fulfill({
        status: 400,
        contentType: 'application/json',
