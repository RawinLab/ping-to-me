...
test.describe('Edit Link Modal - Opening Modal', () => {
  ...
  test('EL-001: Can open edit modal from pencil icon in list view', async ({ page }) => {
    // Wait for links to load
    await page.waitForSelector('[data-testid="link-card"], .group.bg-white.rounded-2xl');
    ...
    const editButton = linkCard.locator('button').filter({ has: page.locator('svg.lucide-pencil') }).first();
  ...
  test('EL-002: Can open edit modal from dropdown menu in list view', async ({ page }) => {
    // Wait for links to load
    ...
    const moreButton = linkCard.locator('button').filter({ has: page.locator('svg.lucide-more-horizontal') }).first();
  ...
  test('EL-003: Can open edit modal from grid view', async ({ page }) => {
    ...
    await page.waitForSelector('.grid.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-3');

    // Hover over first grid card
    const gridCard = page.locator('.grid.grid-cols-1 > div').first().locator('.group');
    await gridCard.hover();

    // Click more options and then edit
    const moreButton = gridCard.locator('button').filter({ has: page.locator('svg.lucide-more-horizontal') }).first();
    await moreButton.click();
  ...
  test('EL-004: Modal displays all form fields', async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Destination URL")')).toBeVisible();
...
test.describe('Edit Link Modal - Destination URL', () => {
  ...
  test('EL-010: Can edit destination URL', async ({ page }) => {
    ...
    // Verify input value
    await expect(urlInput).toHaveValue('https://new-destination.example.com/page');
  });
  ...
  test('EL-012: URL field has red border on validation error', async ({ page }) => {
    // Open edit modal
    const linkCard = page.locator('.group.bg-white.rounded-2xl').first();
    await linkCard.hover();
    const editButton = linkCard.locator('button').filter({ has: page.locator('svg.lucide-pencil') }).first();
...
test.describe('Edit Link Modal - Tags', () => {
  ...
  test('EL-030: Tags section is visible', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Select existing tag")')).toBeVisible();
  });
  ...
  test('EL-031: Can add a new tag', async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"] .flex.items-center.gap-1:has-text("new-test-tag")')).toBeVisible();
  });
  ...
  test('EL-032: Can add tag by pressing Enter', async ({ page }) => {
    ...
    // Tag should appear
    await expect(page.locator('[role="dialog"] .flex.items-center.gap-1:has-text("enter-tag")')).toBeVisible();
  ...
  test('EL-033: Can remove a tag', async ({ page }) => {
    ...
    const tagBadge = page.locator('[role="dialog"] .flex.items-center.gap-1:has-text("remove-me")');
  ...
  test('EL-034: Can select existing tag from dropdown', async ({ page }) => {
    ...
    if (await firstTag.isVisible()) {
      ...
      if (tagText) {
        await expect(page.locator(`[role="dialog"] .flex.items-center.gap-1:has-text("${tagText.trim()}")`)).toBeVisible();
      }
    }
  });
  ...
  test('EL-035: Cannot add duplicate tag', async ({ page }) => {
    ...
    const tagBadges = page.locator('[role="dialog"] .flex.items-center.gap-1:has-text("duplicate-tag")');
...
test.describe('Edit Link Modal - Campaign', () => {
  ...
  test('EL-041: Can open campaign dropdown', async ({ page }) => {
    ...
    await expect(page.locator('[role="option"]:has-text("No Campaign")')).toBeVisible();
  });
...
test.describe('Edit Link Modal - Save & Cancel', () => {
  ...
  test('EL-060: Cancel button closes modal without saving', async ({ page }) => {
    // Open edit modal
    const linkCard = page.locator('.group.bg-white.rounded-2xl').first();
    await linkCard.hover();
    const editButton = linkCard.locator('button').filter({ has: page.locator('svg.lucide-pencil') }).first();
  ...
  test('EL-061: Clicking outside modal closes it', async ({ page }) => {
    ...
    await page.locator('[data-radix-portal] > div').first().click({ position: { x: 10, y: 10 } });
  ...
  test('EL-063: Successful save closes modal and refreshes list', async ({ page }) => {
    // Open edit modal
    const linkCard = page.locator('.group.bg-white.rounded-2xl').first();
    await linkCard.hover();
    const editButton = linkCard.locator('button').filter({ has: page.locator('svg.lucide-pencil') }).first();
...
test.describe('Edit Link Modal - Form Icons', () => {
  ...
  test('EL-070: Form fields have appropriate icons', async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Destination URL") svg.lucide-link-2')).toBeVisible();
    await expect(page.locator('label:has-text("Tags") svg.lucide-tag')).toBeVisible();
    await expect(page.locator('label:has-text("Campaign") svg.lucide-folder-open')).toBeVisible();
    await expect(page.locator('label:has-text("Expiration Date") svg.lucide-calendar')).toBeVisible();
  });
});
