...
test.describe('Links Page - Layout & Navigation', () => {
  ...
  test('LP-001: Page loads with correct header and elements', async ({ page }) => {
    // Check header
    ...
    await expect(page.locator('a[href="/dashboard/links/new"]').first()).toBeVisible();
  });
  ...
  test('LP-003: Filter buttons are visible', async ({ page }) => {
    // Date filter button
    await expect(page.locator('button:has-text("Filter by created date")')).toBeVisible();
  ...
  test('LP-004: Toolbar elements are visible', async ({ page }) => {
    ...
    const toolbarTagButton = page.locator('.flex.justify-between.items-center.py-2 button:has-text("Tag")');
...
test.describe('Links Page - View Mode Toggle', () => {
  ...
  test('LP-010: View mode toggle is visible with three options', async ({ page }) => {
    // Check for view mode toggle container
    const viewToggle = page.locator('.flex.items-center.border.border-slate-200.rounded-lg.bg-white.p-1');
...
test.describe('Links Page - Date Filter Modal', () => {
  ...
  test('LP-031: Date filter modal has calendar', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ has: page.locator('svg.lucide-chevron-left') })).toBeVisible();
    await expect(page.locator('button').filter({ has: page.locator('svg.lucide-chevron-right') })).toBeVisible();
  ...
  test('LP-035: Can clear date filter', async ({ page }) => {
    ...
    await page.click('button:has-text("Filter by created date") ~ button, button:has-text("-") >> button');
...
test.describe('Links Page - Filters Modal', () => {
  ...
  test('LP-043: Filters modal has QR Code dropdown', async ({ page }) => {
    ...
    await expect(page.locator('label:has-text("Attached QR Code")')).toBeVisible();
  });
  ...
  test('LP-044: Can open tags dropdown', async ({ page }) => {
    ...
    const tagsDropdown = page.locator('.absolute.z-10.w-full.mt-1.bg-white.border');
  ...
  test('LP-045: Link type has correct options', async ({ page }) => {
    ...
    await expect(page.locator('[role="option"]:has-text("Standard links")')).toBeVisible();
    await expect(page.locator('[role="option"]:has-text("Bio page links")')).toBeVisible();
  });
  ...
  test('LP-046: QR Code filter has correct options', async ({ page }) => {
    ...
    await expect(page.locator('[role="option"]:has-text("Links with or without attached QR Codes")')).toBeVisible();
    await expect(page.locator('[role="option"]:has-text("Links with attached QR Codes")')).toBeVisible();
    await expect(page.locator('[role="option"]:has-text("Links without attached QR Codes")')).toBeVisible();
  });
  ...
  test('LP-047: Can apply filters', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("1 filter active")')).toBeVisible();
  });
  ...
  test('LP-049: Filters button shows active filter count', async ({ page }) => {
    ...
    const qrSelect = page.locator('button[role="combobox"]').nth(1);
    await qrSelect.click();
    await page.click('[role="option"]:has-text("Links with attached QR Codes")');

    await page.click('button:has-text("Apply")');

    // Should show "2 filters active"
    await expect(page.locator('button:has-text("2 filters active")')).toBeVisible();
  });
});
...
test.describe('Links Page - Link Cards', () => {
  ...
  test('LP-061: Link cards have copy button', async ({ page }) => {
    ...
    if (await copyButton.isVisible()) {
      ...
      await expect(page.locator('svg.lucide-check-circle-2').first()).toBeVisible({ timeout: 2000 });
    }
  });
  ...
  test('LP-063: Can click Add tag button to show dropdown', async ({ page }) => {
    await page.waitForTimeout(1000);
    ...
    if (await addTagButton.isVisible()) {
      ...
      await expect(page.locator('select:has-text("Select tag")').or(page.locator('option:has-text("Select tag")'))).toBeVisible();
    }
  });
  ...
  test('LP-065: Dropdown menu has Edit link option', async ({ page }) => {
    ...
    if (await moreButton.isVisible()) {
      await moreButton.click();
      await expect(page.locator('[role="menuitem"]:has-text("Edit link")')).toBeVisible();
    }
  });
  ...
  test('LP-066: Dropdown menu has QR Code option', async ({ page }) => {
    ...
    if (await moreButton.isVisible()) {
      await moreButton.click();
      await expect(page.locator('[role="menuitem"]:has-text("QR Code")')).toBeVisible();
    }
  });
...
test.describe('Links Page - Selection & Bulk Actions', () => {
  ...
  test('LP-071: Selection shows bulk action bar', async ({ page }) => {
    ...
    if (await checkbox.isVisible()) {
      ...
      await expect(page.locator('.bg-slate-900.text-white.rounded-lg')).toBeVisible();
    }
  });
  ...
  test('LP-072: Bulk action bar has Tag button', async ({ page }) => {
    ...
    if (await checkbox.isVisible()) {
      ...
      await expect(page.locator('.bg-slate-900 button:has-text("Tag")')).toBeVisible();
    }
  });
  ...
  test('LP-073: Bulk action bar has Delete button', async ({ page }) => {
    ...
    if (await checkbox.isVisible()) {
      ...
      await expect(page.locator('.bg-slate-900 button:has-text("Delete")')).toBeVisible();
    }
  });

  test('LP-074: Tag toolbar button is disabled when none selected', async ({ page }) => {
    // Tag button in toolbar should be disabled
    const tagButton = page.locator('button:has-text("Tag")').filter({ hasNot: page.locator('.bg-slate-900') });
...
test.describe('Links Page - Grid View', () => {
  ...
  test('LP-084: Grid cards have Add tag button', async ({ page }) => {
    const gridCard = page.locator('.grid > div').first();
    if (await gridCard.isVisible()) {
      await expect(gridCard.locator('button:has-text("Add tag")')).toBeVisible();
    }
  });
});
...
test.describe('Links Page - Empty State', () => {
  test('LP-090: Shows empty state when no links (mocked)', async ({ page }) => {
    ...
    await expect(page.locator('text=No links yet')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('text=Create your first short link')).toBeVisible();
    await expect(page.locator('a:has-text("Create link")').first()).toBeVisible();
  });
});
