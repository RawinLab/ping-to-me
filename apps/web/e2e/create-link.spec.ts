...
test.describe('Create Link Page', () => {
  ...
  function createLinkResponse(postData: any, customSlug?: string) {
    const slug = customSlug || postData.slug || `link-${Math.random().toString(36).substring(7)}`;
    return {
  ...
  test.describe('Page Navigation & Layout', () => {
    test('should navigate to create link page', async ({ page }) => {
      ...
      await expect(page.locator('h1:has-text("Create a new link")')).toBeVisible();
    });
    ...
    test('should have Cancel and Create buttons', async ({ page }) => {
      ...
      await expect(page.locator('button:has-text("Create your link")')).toBeVisible();
    });
  });

  test.describe('Link Details Section', () => {
    test('CL-001: should create link with destination URL only', async ({ page }) => {
      await setupAuthenticatedState(page);
    ...
    test('CL-003: should show error for duplicate slug', async ({ page }) => {
      ...
      await page.fill('input#originalUrl', validUrl);
      await page.fill('input[placeholder="custom-slug (optional)"]', 'taken-slug');

      await page.click('button:has-text("Create your link")');

      // Should show error message
      await expect(page.locator('text=This slug is already taken')).toBeVisible({ timeout: 5000 });
    });
    ...
    test('CL-007: should show custom domain in dropdown', async ({ page }) => {
      ...
      await expect(page.getByRole('option', { name: 'custom.link' })).toBeVisible();
    });
  });
  ...
  test.describe('Sharing Options - QR Code', () => {
    ...
    test('CL-012: should change QR color and update preview', async ({ page }) => {
      await setupAuthenticatedState(page);
    ...
    test('CL-013: should upload logo for QR code', async ({ page }) => {
      ...
      await expect(page.locator('img[alt="Logo preview"]').first()).toBeVisible({ timeout: 5000 });
    });
    ...
    test('CL-014: should remove uploaded logo', async ({ page }) => {
      ...
      const logoContainer = page.locator('img[alt="Logo preview"]').first().locator('..');
    ...
    test('CL-015: should show QR preview when URL is entered', async ({ page }) => {
      await setupAuthenticatedState(page);
    ...
    test('CL-016: should create link with custom QR color', async ({ page }) => {
      await setupAuthenticatedState(page);
  ...
  test.describe('Sharing Options - Bio Page', () => {
    ...
    test('CL-021: should select bio page from dropdown', async ({ page }) => {
      ...
      await page.locator('[role="combobox"]:has-text("Select a bio page")').click();
  ...
  test.describe('Advanced Settings', () => {
    test('CL-030: should expand advanced settings section', async ({ page }) => {
      await setupAuthenticatedState(page);
      await page.goto('/dashboard/links/new');
    ...
    test('CL-036: should set deep link fallback', async ({ page }) => {
      ...
      await page.route('**/links', async route => {
        if (route.request().method() === 'POST') {
          const postData = route.request().postDataJSON();
          expect(postData.deepLinkFallback).toBe('https://app.example.com/fallback');
          await route.fulfill({
            status: 201,
            contentType: 'application/json',
            body: JSON.stringify(createLinkResponse(postData))
      ...
      await page.fill('input#deepLinkFallback', 'https://app.example.com/fallback');
  ...
  test.describe('Success State', () => {
    test('CL-040: should display success state after link creation', async ({ page }) => {
      await setupAuthenticatedState(page);

      const mockLink = createLinkResponse({ originalUrl: validUrl }, 'test-slug');
    ...
    test('CL-041: should display short URL in success state', async ({ page }) => {
      await setupAuthenticatedState(page);

      const mockLink = createLinkResponse({ originalUrl: validUrl }, 'my-test-link');
      ...
      await expect(page.locator('text=Link Created!')).toBeVisible({ timeout: 10000 });
      await expect(page.locator('text=localhost:3000/my-test-link')).toBeVisible();
    });

    test('CL-042: should copy short URL to clipboard', async ({ page, context }) => {
      await setupAuthenticatedState(page);

      // Grant clipboard permission
      await context.grantPermissions(['clipboard-read', 'clipboard-write']);

      const mockLink = createLinkResponse({ originalUrl: validUrl }, 'copy-test');
    ...
    test('CL-043: should display QR code in success state', async ({ page }) => {
      await setupAuthenticatedState(page);
    ...
    test('CL-044: should have Customize QR button in success state', async ({ page }) => {
      await setupAuthenticatedState(page);

      const mockLink = createLinkResponse({ originalUrl: validUrl }, 'customize-test');
      ...
      await expect(page.locator('button:has-text("Customize QR")')).toBeVisible();
    });

    test('CL-045: should create another link from success state', async ({ page }) => {
      await setupAuthenticatedState(page);
      ...
      await page.route('**/links', async route => {
        if (route.request().method() === 'POST') {
          ...
          await route.fulfill({
            status: 201,
            contentType: 'application/json',
            body: JSON.stringify(createLinkResponse(route.request().postDataJSON()))
          });
        }
      });
    ...
    test('CL-046: should navigate to analytics from success state', async ({ page }) => {
      await setupAuthenticatedState(page);

      const mockLink = createLinkResponse({ originalUrl: validUrl }, 'analytics-test');
  ...
  test.describe('Form Validation & Edge Cases', () => {
    test('CL-050: should show loading state during submission', async ({ page }) => {
      await setupAuthenticatedState(page);
    ...
    test('CL-052: should handle blocked domain error', async ({ page }) => {
      ...
      await expect(page.locator('text=This domain is blocked')).toBeVisible({ timeout: 5000 });
    });
    ...
    test('CL-054: should preserve form state when toggling sections', async ({ page }) => {
      await setupAuthenticatedState(page);
      await page.goto('/dashboard/links/new');
  ...
  test.describe('Complete Flow', () => {
    test('CL-060: should create link with all options', async ({ page }) => {
      ...
      await page.fill('input#originalUrl', validUrl);
      await page.fill('input[placeholder="custom-slug (optional)"]', `full-test-${randomId}`);
      ...
      await page.fill('input#expirationDate', tomorrow.toISOString().slice(0, 16));
