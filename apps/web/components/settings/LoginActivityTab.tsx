...
const isSuspiciousActivity = (
  ...
  if (!activity.success) {
    const recentFailures = activities.filter(
      (a) =>
        !a.success &&
        new Date(a.createdAt).getTime() > new Date().getTime() - 3600000 &&
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {activities.map((activity) => {
                const browser =
                  activity.browser ||
                  parseUserAgent(activity.userAgent || "").browser;
                const os =
                  activity.os ||
                  parseUserAgent(activity.userAgent || "").os;
                const suspicious = isSuspiciousActivity(activity, activities);
                  ...
                  >
                    <div
                      className={`h-12 w-12 rounded-lg flex items-center justify-center flex-shrink-0 ${
                        activity.success ? "bg-emerald-100" : "bg-red-100"
                      }`}
                    >
                      {activity.success ? (
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <h4
                          className={`font-medium ${
                            activity.success ? "text-slate-900" : "text-red-900"
                          }`}
                        >
                          {activity.success
                            ? "Successful Login"
