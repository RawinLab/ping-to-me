...
export default function DomainDetailsPage() {
  ...
  const handleSetDefault = async () => {
    if (!domain || !currentOrg?.id) return;
    if (!confirm(t("setDefaultConfirm", { hostname: domain.hostname }))) return;
    setActionLoading(true);
    try {
  ...
  const handleDelete = async () => {
    if (!domain || !currentOrg?.id) return;
    if (
      !confirm(t("deleteDomainConfirm", { hostname: domain.hostname }))
    )
      return;
    setActionLoading(true);
    try {
      ...
      case "FAILED":
        return (
          <Badge className="bg-red-100 text-red-700 hover:bg-red-100 border-0">
            <AlertTriangle className="mr-1 h-3 w-3" /> {t("failed")}
          </Badge>
        );
      case "PENDING":
      default:
        return (
          <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 border-0">
            <Clock className="mr-1 h-3 w-3" /> {t("pendingVerification")}
          </Badge>
        );
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-500 mt-1">{t("domainDetails")}</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {domain.isDefault && (
                  <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100 border-0">
                    <Star className="mr-1 h-3 w-3 fill-blue-700" /> {t("default")}
                  </Badge>
                )}
              </div>
            </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-slate-500 mb-1">{t("hostname")}</p>
                <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm text-slate-500 mb-1">{t("addedDate")}</p>
              ...
              {domain.lastVerifiedAt && (
                <div>
                  <p className="text-sm text-slate-500 mb-1">{t("lastVerified")}</p>
                  <p className="text-sm font-medium">
                    {format(
            ...
            <div className="flex flex-wrap gap-2 pt-4 border-t">
              {!domain.isVerified && (
                ...
                >
                  ...
                  {domain.status === "FAILED" ? t("retryVerification") : t("verifyDomain")}
                </Button>
              )}
          ...
          <Card data-testid="domain-analytics-card">
            ...
            <CardContent>
              {analyticsLoading ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4">
                      <p className="text-sm text-blue-600 font-medium">{t("totalClicks")}</p>
                      ...
                      <p className="text-xs text-blue-500 mt-1">{t("last30Days")}</p>
                    </div>
                    <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-xl p-4">
                      <p className="text-sm text-emerald-600 font-medium">{t("totalLinks")}</p>
                      <p className="text-3xl font-bold text-emerald-700 mt-1">
                        {analytics.totalLinks.toLocaleString()}
                      </p>
                      <p className="text-xs text-emerald-500 mt-1">{t("usingThisDomain")}</p>
                    </div>
                    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4">
                      <p className="text-sm text-purple-600 font-medium">{t("avgClicksPerLink")}</p>
                      <p className="text-3xl font-bold text-purple-700 mt-1">
                        {analytics.totalLinks > 0
                          ? Math.round(analytics.totalClicks / analytics.totalLinks)
                          : 0}
                      </p>
                      <p className="text-xs text-purple-500 mt-1">{t("performanceMetric")}</p>
                    </div>
                  </div>
                      ...
                      <div className="space-y-2">
                        {analytics.topLinks.map((link, index) => (
                          ...
                          >
                            ...
                            <Badge variant="outline">{t("clicksCount", { count: link.clicks })}</Badge>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <p className="text-sm text-slate-500">{t("noAnalyticsData")}</p>
              )}
            </CardContent>
          </Card>
        )}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium">{t("txtRecordRecommended")}</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium">{t("cnameRecord")}</p>
                </div>
                <div className="bg-slate-50 rounded-lg border p-4 space-y-2">
                  <div className="grid grid-cols-3 gap-2 text-xs text-slate-500 font-medium">
                      <span>{t("dnsType")}</span>
              ...
              {domain.status === "FAILED" && domain.verificationError && (
                <div className="text-sm text-red-700 bg-red-50 rounded-lg px-4 py-3 border border-red-200">
                  <strong>{t("verificationFailedError")}</strong> {domain.verificationError}
                </div>
              )}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline">{t("linksCount", { count: links.length })}</Badge>
            </div>
          </CardHeader>
          <CardContent>
            {linksLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 mb-4">
                  {t("noLinksUsingDomain")}
                </p>
                <Button asChild>
                  <Link href="/dashboard/links/new">{t("createLink")}</Link>
                </Button>
              </div>
            ) : (
              <div className="space-y-2">
