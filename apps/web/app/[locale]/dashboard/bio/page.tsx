...
export default function BioPagesDashboard() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("bioPageDeleted"));
      fetchPages();
    } catch (error) {
      console.error("Failed to delete bio page", error);
      toast.error(t("failedToDeleteBioPage"));
    } finally {
  ...
  const EmptyState = () => (
    <div className="col-span-full flex flex-col items-center justify-center py-16 px-4">
      ...
      <h3 className="text-2xl font-semibold mb-2">
        {t("createFirstBioPage")}
      </h3>
      <p className="text-muted-foreground text-center max-w-md mb-6">
        {t("bioPageDescription")}
      </p>
      <Button
        size="lg"
        onClick={() => {
      ...
      <div className="p-6 lg:p-8 space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Button
            ...
            className="gap-2"
          >
            <Plus className="h-4 w-4" /> {t("createPage")}
          </Button>
        </div>
                  ...
                  <CardContent className="space-y-4">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        <span>{page.viewCount || 0} {t("views")}</span>
                      </div>
                    </div>
                    ...
                    >
                      <Edit className="h-4 w-4" /> {t("editPage")}
                    </Button>
                  </CardContent>
                </Card>
              </TooltipProvider>
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t("deleteConfirmDescription", { title: pageToDelete?.title })}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">
          {view === "create" ? t("createBioPage") : t("editBioPage")}
        </h1>
      </div>
