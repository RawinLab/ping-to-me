...
function ProfileSettingsContent() {
  ...
  const handleUnlinkAccount = async (provider: string) => {
    if (
      !window.confirm(t("unlinkConfirm", { provider }))
    ) {
      return;
    }
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: t("unlinked", { provider }),
      });
      fetchLinkedAccounts();
    } catch (error: any) {
      setMessage({
        type: "error",
        text: error.message || t("unlinkFailed"),
      });
    } finally {
            ...
            <Card className="border-slate-200 shadow-sm overflow-hidden">
              <CardHeader className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <CardTitle className="text-lg">{t("notificationSettings")}</CardTitle>
                    <CardDescription>
