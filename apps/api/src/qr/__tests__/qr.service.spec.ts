...
describe("QrCodeService", () => {
  ...
  describe("getQrConfig", () => {
    ...
    it("should return config for existing link", async () => {
      (prismaService.qrCode.findUnique as jest.Mock).mockResolvedValue(
        mockQrConfig,
      );
  ...
  describe("saveQrConfig", () => {
    ...
    it("should throw NotFoundException for non-existent link", async () => {
      ...
      await expect(
        service.saveQrConfig("non-existent", createDto),
  ...
  describe("batchGenerateQr", () => {
    ...
    it("should generate ZIP with all QRs in PNG format", async () => {
      ...
      const result = await service.batchGenerateQr(
        ["link-1", "link-2"],
        "png",
        300,
      );
    ...
    it("should throw NotFoundException when no links found", async () => {
      ...
      await expect(
        service.batchGenerateQr(["non-existent"], "png", 300),
      ).rejects.toThrow(NotFoundException);
      await expect(
        service.batchGenerateQr(["non-existent"], "png", 300),
      ).rejects.toThrow("No links found");
    });
  ...
  describe("deleteQrConfig", () => {
    it("should delete QR config for a link", async () => {
      (prismaService.qrCode.deleteMany as jest.Mock).mockResolvedValue({
        count: 1,
      });
  ...
  describe("generateQrCode", () => {
    beforeEach(() => {
      ...
      (storageService.uploadFile as jest.Mock).mockResolvedValue(
        "https://cdn.example.com/qr/test-slug.png",
      );
    });
    ...
    it("should throw error when QR generation fails", async () => {
      ...
      await expect(
        service.generateQrCode("https://example.com", "test-slug"),
  ...
  describe("uploadLogoToR2", () => {
    beforeEach(() => {
      (storageService.uploadFile as jest.Mock).mockResolvedValue(
        "https://cdn.example.com/qr-logos/link-123.png",
      );
    });
    ...
    it("should throw BadRequestException when upload fails", async () => {
      ...
      await expect(
        service.uploadLogoToR2(base64Logo, "link-123"),
