...
describe("PermissionMatrix", () => {
  ...
  describe("hasPermission() Function", () => {
    ...
    it("should return false for invalid role", () => {
      expect(hasPermission("INVALID" as MemberRole, "link", "create")).toBe(
        false,
      );
    });
  ...
  describe("Permission Matrix Consistency", () => {
    ...
    it("should have all scopes as valid PermissionScope type", () => {
      const validScopes: PermissionScope[] = [
        "*",
        "own",
        "organization",
        "limited",
    ...
    it("should have well-defined role hierarchy in permissions", () => {
      ...
      const ownerPermCount = Object.values(
        PERMISSION_MATRIX[MemberRole.OWNER],
      ).length;
      const adminPermCount = Object.values(
        PERMISSION_MATRIX[MemberRole.ADMIN],
      ).length;
      const editorPermCount = Object.values(
        PERMISSION_MATRIX[MemberRole.EDITOR],
      ).length;
      const viewerPermCount = Object.values(
        PERMISSION_MATRIX[MemberRole.VIEWER],
      ).length;
  ...
  describe("Edge Cases and Boundary Tests", () => {
    ...
    it("should handle mixed scope arrays properly", () => {
      const adminApiKeyPerms = getPermissions(
        MemberRole.ADMIN,
        "api-key",
        "read",
      );
  ...
  describe("Authorization Rule Enforcement", () => {
    ...
    it("should prevent privilege escalation attempts", () => {
      ...
      expect(hasPermission(MemberRole.EDITOR, "team", "update-role")).toBe(
        false,
      );

      // VIEWER cannot manage team at all
      expect(hasPermission(MemberRole.VIEWER, "team", "update-role")).toBe(
        false,
      );
    });
    ...
    it("should enforce resource-specific restrictions", () => {
      ...
      expect(hasPermission(MemberRole.OWNER, "organization", "delete")).toBe(
        true,
      );
      expect(hasPermission(MemberRole.ADMIN, "organization", "delete")).toBe(
        false,
      );
      expect(hasPermission(MemberRole.EDITOR, "organization", "delete")).toBe(
        false,
      );
      expect(hasPermission(MemberRole.VIEWER, "organization", "delete")).toBe(
        false,
      );
